...
public class HS100 implements DeviceInterface {
	...
	public boolean isPresent(int timeout) {
		...
		if (!present) {
			try {
				...
				present = ip.isReachable(timeout);
			} catch (IOException ex) {
				present = false;
			}
		}
		if (present) {
			try {
				present = !"".equals(sendCommand(COMMAND_INFO));
			} catch (Exception ex) {
				present = false;
			}
		}
	...
	public boolean nightModeOn() throws IOException {
		...
		if (jsonData.length() > 0) {
			...
			JsonObject jo = new JsonParser().parse(jsonData).getAsJsonObject();
			int errorCode = jo.get("system").getAsJsonObject().get("set_led_off").getAsJsonObject().get("err_code")
					.getAsInt();
			return errorCode == 0;
		}
		return false;
	...
	public int readState() throws IOException {
		...
		if (jsonData.length() > 0) {
			...
			JsonObject jo = new JsonParser().parse(jsonData).getAsJsonObject();
			int state = jo.get("system").getAsJsonObject().get("get_sysinfo").getAsJsonObject().get("relay_state")
					.getAsInt();
